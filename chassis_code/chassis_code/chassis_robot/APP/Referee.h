/**
  ******************************************************************************
  * @file    referee.h
  * @author  圣灵亮欣
  * @version V1.7.0
  * @date    2025/01/08
  * @brief   Header file of referee.c
  ******************************************************************************
  * @attention
	*
	*   依据裁判系统 串口协议附录 V1.7
	*
  ******************************************************************************
  */
	
/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __REFEREE_H__
#define __REFEREE_H__

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stdint.h"
#include "protocol.h"
#include "fifo.h"

/* Defines -------------------------------------------------------------------*/
#define Referee_UART huart6

/* Referee Defines -----------------------------------------------------------*/
/* 比赛类型 */
#define Game_Type_RMUC     1 //超级对抗赛
#define Game_Type_RMUT     2 //单项赛
#define Game_Type_RMUA     3 //人工智能挑战赛
#define Game_Type_RMUL_3V3 4 //高校联盟赛3V3
#define Game_Type_RMUL_1V1 5 //高校联盟赛1V1

/* 比赛阶段 */
#define Game_Progress_Unstart   0 //未开始比赛
#define Game_Progress_Prepare   1 //准备阶段
#define Game_Progress_SelfCheck 2 //自检阶段
#define Game_Progress_5sCount   3 //5s倒计时
#define Game_Progress_Battle    4 //对战中
#define Game_Progress_Calculate 5 //比赛结算中

/* 比赛结果 */
#define Game_Result_Draw    0 //平局
#define Game_Result_RedWin  1 //红方胜利
#define Game_Result_BlueWin 2 //蓝方胜利

/* 警告信息 */
#define Warning_Yellow  1 //黄牌警告
#define Warning_Red     2 //红牌警告
#define Warning_Failure 3 //判负

/* 机器人ID */
#define Robot_ID_Red_Hero         1 //红方英雄
#define Robot_ID_Red_Engineer     2 //红方工程
#define Robot_ID_Red_Infantry3    3 //红方步兵3
#define Robot_ID_Red_Infantry4    4 //红方步兵4
#define Robot_ID_Red_Infantry5    5 //红方步兵5
#define Robot_ID_Red_Aerial       6 //红方无人机
#define Robot_ID_Red_Sentry       7 //红方哨兵
#define Robot_ID_Red_Darts        8 //红方飞镖
#define Robot_ID_Red_Radar        9 //红方雷达
#define Robot_ID_Blue_Hero      101 //蓝方英雄
#define Robot_ID_Blue_Engineer  102 //蓝方工程
#define Robot_ID_Blue_Infantry3 103 //蓝方步兵3
#define Robot_ID_Blue_Infantry4 104 //蓝方步兵4
#define Robot_ID_Blue_Infantry5 105 //蓝方步兵5
#define Robot_ID_Blue_Aerial    106 //蓝方无人机
#define Robot_ID_Blue_Sentry    107 //蓝方哨兵
#define Robot_ID_Blue_Darts     108 //蓝方飞镖
#define Robot_ID_Blue_Radar     109 //蓝方雷达

/* 扣血类型 */
#define Hurt_Type_ArmoredPlate     0 //弹丸攻击伤害
#define Hurt_Type_ModuleOffline    1 //重要模块离线
#define Hurt_Type_OverShootSpeed   2 //射击初速超限
#define Hurt_Type_OverShootHeat    3 //射击超热量
#define Hurt_Type_OverChassisPower 4 //底盘超功率
#define Hurt_Type_Collision        5 //装甲受撞击

/* 发射机构编号 */
#define Shooter_ID1_17mm		1 //1号17mm发射机构
#define Shooter_ID2_17mm		2 //2号17mm发射机构
#define Shooter_ID1_42mm		3 //1号42mm发射机构

/* 飞镖信息 */
#define Dart_State_Open     0 //飞镖闸门已开启
#define Dart_State_Close    1 //飞镖闸门关闭
#define Dart_State_Changing 2 //正在开启或者关闭中

/* 操作手ID */
#define Cilent_ID_Red_Hero       0x0101 //红方英雄操作手
#define Cilent_ID_Red_Engineer   0x0102 //红方工程操作手
#define Cilent_ID_Red_Infantry3  0x0103 //红方步兵3操作手
#define Cilent_ID_Red_Infantry4  0x0104 //红方步兵4操作手
#define Cilent_ID_Red_Infantry5  0x0105 //红方步兵5操作手
#define Cilent_ID_Red_Aerial     0x0106 //红方云台手
#define Cilent_ID_Blue_Hero      0x0165 //蓝方英雄操作手
#define Cilent_ID_Blue_Engineer  0x0166 //蓝方工程操作手
#define Cilent_ID_Blue_Infantry3 0x0167 //蓝方步兵3操作手
#define Cilent_ID_Blue_Infantry4 0x0168 //蓝方步兵4操作手
#define Cilent_ID_Blue_Infantry5 0x0169 //蓝方步兵5操作手
#define Cilent_ID_Blue_Aerial    0x016A //蓝方云台手

/* 0x00XX --------------------------------------------------------------------*/
typedef struct __attribute__((packed))  //*0x0001 比赛状态数据（服务器→全体机器人，常规链路）
{
	uint8_t  game_type : 4;
	uint8_t  game_progress : 4;
	uint16_t stage_remain_time;
	uint64_t SyncTimeStamp;
} ext_game_status_t;

typedef struct __attribute__((packed))  //0x0002 比赛结果数据（服务器→全体机器人，常规链路）
{
	uint8_t winner;
} ext_game_result_t;

typedef struct __attribute__((packed))  //*0x0003 机器人血量数据（服务器→全体机器人，常规链路）
{
	uint16_t red_1_robot_HP;
	uint16_t red_2_robot_HP;
	uint16_t red_3_robot_HP;
	uint16_t red_4_robot_HP;
	uint16_t reserved_1;
	uint16_t red_7_robot_HP;
	uint16_t red_outpost_HP;
	uint16_t red_base_HP;
	uint16_t blue_1_robot_HP;
	uint16_t blue_2_robot_HP;
	uint16_t blue_3_robot_HP;
	uint16_t blue_4_robot_HP;
	uint16_t reserved_2;
	uint16_t blue_7_robot_HP;
	uint16_t blue_outpost_HP;
	uint16_t blue_base_HP;
} ext_game_robot_HP_t;

/* 0x01XX --------------------------------------------------------------------*/
typedef struct __attribute__((packed))  //0x0101 场地事件数据（服务器→己方全体机器人，常规链路）
{
  uint32_t event_type;
} ext_event_data_t;

typedef struct __attribute__((packed))  //0x0104 裁判警告信息（服务器→被判罚方全体机器人，常规链路）
{
	uint8_t level; 
	uint8_t offending_robot_id; 
	uint8_t count;
} ext_referee_warning_t;

typedef struct __attribute__((packed))	//0x0105 飞镖发射数据（服务器→己方全体机器人，常规链路）
{
	uint8_t dart_remaining_time;
	uint16_t dart_info;
} ext_dart_info_t;

typedef struct __attribute__((packed))	//0x0120	哨兵自主决策指令（哨兵机器人→服务器，常规链路）
{ 
 uint32_t sentry_cmd; 
} ext_sentry_cmd_t;

typedef struct __attribute__((packed))	//0x0121	雷达自主决策指令（雷达系统→服务器，常规链路）
{ 
 uint8_t radar_cmd; 
} ext_radar_cmd_t;

/* 0x02XX --------------------------------------------------------------------*/
typedef struct __attribute__((packed))	//0x0201 机器人性能体系数据（主控模块→对应机器人，常规链路）
{
	uint8_t  robot_id;
	uint8_t  robot_level;
	uint16_t current_HP;
	uint16_t maximun_HP;
	uint16_t shooter_barrel_cooling_value;
	uint16_t shooter_barrel_heat_limit;
	uint16_t chassis_power_limit;
	
	uint8_t  power_management_gimbal_output  : 1;
	uint8_t  power_management_chassis_output : 1;
	uint8_t  power_management_shooter_output : 1;
} ext_robot_status_t;

typedef struct __attribute__((packed))  //*0x0202 实时底盘 缓冲能量和射击热量（主控模块→对应机器人，常规链路）
{
  uint16_t reserved_1; 
  uint16_t reserved_2; 
  float reserved_3; 
  uint16_t buffer_energy; 
  uint16_t shooter_17mm_1_barrel_heat; 
  uint16_t shooter_17mm_2_barrel_heat; 
  uint16_t shooter_42mm_barrel_heat; 
} ext_power_heat_data_t;

typedef struct __attribute__((packed))  //0x0203 机器人位置（主控模块→对应机器人，常规链路）
{
	uint32_t x;
	uint32_t y;
	uint32_t angle;
} ext_robot_pos_t;

typedef struct __attribute__((packed))  //*0x0204 机器人增益和底盘能量（服务器→对应机器人，常规链路）
{
  uint8_t recovery_buff;  
  uint8_t cooling_buff;  
  uint8_t defence_buff;  
  uint8_t vulnerability_buff; 
  uint16_t attack_buff;
	uint8_t remaining_energy;
} ext_buff_energy_t; 

typedef struct __attribute__((packed))  //0x0206 伤害状态（主控模块→对应机器人，常规链路）
{
	uint8_t armor_id : 4; 
	uint8_t HP_deduction_reason : 4;
} ext_hurt_data_t;

typedef struct __attribute__((packed))  //0x0207 实时射击信息（主控模块→对应机器人，常规链路）
{
	uint8_t bullet_type; 
	uint8_t shooter_number; 
	uint8_t launching_frequency; 
	float initial_speed;
} ext_shoot_data_t;

typedef struct __attribute__((packed)) 	//*0x0208 允许发弹量（服务器→己方英雄、步兵、哨兵、空中机器人，常规链路）
{ 
 uint16_t projectile_allowance_17mm;
 uint16_t projectile_allowance_42mm;
 uint16_t remaining_gold_coin;
 uint16_t projectile_allowance_fortress;
} ext_projectile_allowance_t;

typedef struct __attribute__((packed))	//*0x0209 机器人RFID状态（服务器→己方装有RFID的机器人，常规链路）
{
	uint32_t rfid_status;
} ext_rfid_status_t;

typedef struct __attribute__((packed))  //0x020A 飞镖选手端指令数据（服务器→己方飞镖系统，常规链路）
{
	uint8_t  dart_launch_opening_status;
	uint8_t  reserved_1;
	uint16_t target_change_time;
	uint16_t latest_launch_cmd_time;
} ext_dart_client_cmd_t;

typedef struct __attribute__((packed))  //*0x020B  地面机器人位置数据（服务器→己方哨兵机器人，常规链路）
{ 
	float hero_x; 
	float hero_y; 
	float engineer_x; 
	float engineer_y; 
	float standard_3_x; 
	float standard_3_y; 
	float standard_4_x; 
	float standard_4_y; 
	float reserved_1; 
	float reserved_2; 
} ext_ground_robot_position_t;

typedef struct __attribute__((packed))  //0x020C  雷达标记进度数据（服务器→己方雷达系统，常规链路）
{ 
	uint8_t mark_progress; 
} ext_radar_mark_data_t;

typedef struct __attribute__((packed))  //*0x020D  哨兵自主决策信息（服务器→己方哨兵机器人，常规链路）
{ 
	uint32_t sentry_info; 
	uint16_t sentry_info_2; 
} ext_sentry_info_t;

typedef struct __attribute__((packed))  //0x020E  雷达自主决策信息（服务器→己方雷达系统，常规链路）
{ 
	uint8_t radar_info; 
} ext_radar_info_t;
/* 0x03XX --------------------------------------------------------------------*/
typedef struct __attribute__((packed))  //*0x0301 机器人交互数据
{
	uint16_t data_cmd_id;
	uint16_t sender_ID;
	uint16_t receiver_ID;
} ext_robot_interaction_header_t;

typedef struct __attribute__((packed))  //0x0301 机器人交互数据 数据结构体
{
	    uint16_t x;
			uint16_t y;
			uint8_t z;
} robot_position_t;

typedef struct __attribute__((packed))  //0x0301 机器人交互数据 数据结构体
{
	    uint16_t data_cmd_id;
			uint16_t sender_id;
			uint16_t receiver_id;	
			uint8_t radar_enhance;
			robot_position_t enemy[6];
			robot_position_t alliance[6];
} ext_robot_interaction_user_data_t;

typedef struct __attribute__((packed))  //0x0302 自定义控制器与机器人交互数据（自定义控制器→机器人，图传链路）
{
	uint8_t data[30]; 
} ext_custom_robot_data_t;

typedef struct __attribute__((packed))  //*0x0303 选手端小地图交互数据（选手端→服务器→选择的己方机器人，常规链路）
{ 
	uint32_t target_position_x; 
	uint32_t target_position_y; 
	uint8_t cmd_keyboard; 
	uint8_t target_robot_id; 
	uint16_t cmd_source; 
} ext_map_command_t;

typedef struct __attribute__((packed))  //0x0304 键鼠遥控数据（选手端→机器人，图传链路）
{
	int16_t		mouse_x; 
	int16_t		mouse_y; 
	int16_t		mouse_z; 
	int8_t		left_button_down; 
	int8_t		right_button_down; 
	uint16_t	keyboard_value; 
	uint16_t	reserved_1; 
} ext_remote_control_t;

typedef struct __attribute__((packed))  //0x0305 选手端小地图接收雷达数据（雷达→服务器→己方所有选手端，常规链路）
{ 
	uint16_t hero_position_x; 
	uint16_t hero_position_y; 
	uint16_t engineer_position_x; 
	uint16_t engineer_position_y; 
	uint16_t infantry_3_position_x; 
	uint16_t infantry_3_position_y; 
	uint16_t infantry_4_position_x; 
	uint16_t infantry_4_position_y; 
	uint16_t infantry_5_position_x; 
	uint16_t infantry_5_position_y; 
	uint16_t sentry_position_x; 
	uint16_t sentry_position_y; 
} ext_map_robot_data_t;

typedef struct __attribute__((packed))  //0x0306 自定义控制器与选手端交互数据（自定义控制器→选手端）
{
	uint16_t key_value; 
	uint16_t x_position:12; 
	uint16_t mouse_left:4; 
	uint16_t y_position:12;
	uint16_t mouse_right:4; 
	uint16_t reserved_1; 
} ext_custom_client_data_t;

typedef struct __attribute__((packed))  //-0x0307 选手端小地图接收哨兵数据（哨兵/半自动机器人→对应操作手选手端，常规链路）
{
	uint8_t	 intention; 
	uint16_t start_position_x; 
	uint16_t start_position_y; 
	int8_t	 delta_x[49]; 
	int8_t	 delta_y[49]; 
	uint16_t sender_id; 
} ext_map_data_t;

typedef struct __attribute__((packed))  //-0x0308 选手端小地图接收机器人数据（己方机器人→己方选手端，常规链路）
{
	uint16_t sender_id; 
	uint16_t receiver_id; 
	uint8_t user_data[30]; 
} ext_custom_info_t;

typedef struct __attribute__((packed))  //0x0309 自定义控制器接收机器人数据（己方机器人→自定义控制器，图传链路）
{
	uint8_t data[30]; 
} ext_robot_custom_data_t;


/* Structs -------------------------------------------------------------------*/
/* protocol包头结构体 */
extern frame_header_struct_t Referee_Receive_Header;

/* 0x00XX */
extern ext_game_status_t							 		Game_Status;
extern ext_game_result_t							 		Game_Result;
extern ext_game_robot_HP_t						 		Game_Robot_HP;

/* 0x01XX */
extern ext_event_data_t                		Event_Data;
extern ext_referee_warning_t           		Referee_Warning;
extern ext_dart_info_t								 		Dart_Information;
extern ext_sentry_cmd_t										Sentry_Cmd;
extern ext_radar_cmd_t										Radar_Cmd;

/* 0x02XX */
extern ext_robot_status_t 						 		Robot_Status;
extern ext_power_heat_data_t 					 		Power_Heat_Data;
extern ext_robot_pos_t   							 		Robot_Pos;
extern ext_buff_energy_t       						Buff_Energy;
extern ext_hurt_data_t	     					 		Hurt_Data;
extern ext_shoot_data_t      					 		Shoot_Data;
extern ext_projectile_allowance_t			 		Projectile_Allowance;
extern ext_rfid_status_t      				 		RFID_Status;
extern ext_dart_client_cmd_t					 		Dart_Client_Cmd;
extern ext_ground_robot_position_t		 		Ground_Robot_Position;
extern ext_radar_mark_data_t					 		Radar_Mark_Data;
extern ext_sentry_info_t							 		Sentry_Info;
extern ext_radar_info_t								 		Radar_Info;

/* 0x03XX */
extern ext_robot_interaction_header_t			Robot_Interaction_Header_Data;
extern ext_robot_interaction_user_data_t	Robot_Interaction_User_Data;
extern ext_custom_robot_data_t						Custom_Robot_Data;
extern ext_map_command_t									Map_Command;
extern ext_remote_control_t								Remote_Control;
extern ext_map_robot_data_t								Map_Robot_Data;
extern ext_custom_client_data_t						Custom_Client_Data;
extern ext_map_data_t											Map_Data;
extern ext_custom_info_t									Custom_Info;
extern ext_robot_custom_data_t						Robot_Custom_Data;

/* Functions -----------------------------------------------------------------*/
void Referee_StructInit(void);
void Referee_UARTInit(uint8_t *Buffer0, uint8_t *Buffer1, uint16_t BufferLength);

void Referee_UnpackFifoData(unpack_data_t *p_obj, fifo_s_t *referee_fifo);
void Referee_SolveFifoData(uint8_t *frame);

/* User */
extern uint8_t if_hurted;

#ifdef __cplusplus
}
#endif

#endif /* __REFEREE_H__ */
