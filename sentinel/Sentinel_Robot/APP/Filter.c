 ...
 *@return	none
 */
float fliter_num1[3] = {1.929454039488895f, -0.93178349823448126f, 0.002329458745586203f};
...
float ewma_filter_update(WeightedFilter *filter, float input) {
	  ...
	  filter->ewma_value -= filter->oldest * powf((1 - filter->alpha),filter->size-1)  * filter->alpha; 
	  filter->ewma_value += input * powf((1 - filter->alpha),filter->size-1)  * filter->alpha; 

    filter->buffer[filter->index] = input;
    filter->index = (filter->index + 1) % filter->size;
    filter->oldest = filter->buffer[filter->index];

		
    filter->ewma_value = filter->alpha * input + (1 - filter->alpha) * filter->ewma_value;
	
    if( fabs(filter->oldest - filter->ewma_value) < 0.08* fabs(filter->ewma_value))
			return filter->ewma_value;
		else
			return input;
}
